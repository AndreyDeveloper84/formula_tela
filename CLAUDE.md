...
- LandingPageGenerator (agents/agents/landing_generator.py): generate_landing() — генерация из БД; generate_from_markdown() — маркдаун как редакторский бриф; _get_services_context(), _build_prompt(), _build_prompt_with_markdown(), _check_markdown_vs_db(), _make_slug() (dedup -v2/-v3); LandingPage.source_markdown (TextField, миграция 0006); admin action «Сгенерировать из маркдауна» + MarkdownUploadForm + HTML-шаблон; 46 тестов в test_landing_generator.py
- Landing page view + URL + шаблон + templatetags (task 4.2): agents/views.py (landing_page_view), URL `<slug:slug>/` последним в urlpatterns, шаблон agents/landing_page.html, templatetags landing_tags.py (split_lines, slugify_to_title), 37 тестов в test_landing_page_view.py
- landing_page.html CTA-кнопки: data-bs-toggle="modal" data-bs-target="#exampleModal", открывают ту же модалку что и остальные страницы
- LandingBlock модель (task 5.1): копия ServiceBlock с FK на LandingPage; 12 типов блоков (text, accent, checklist, identification, cta, price_table, accordion, faq, special_formats, subscriptions, navigation, html); LandingBlockInline в admin; _create_blocks() в генераторе; BLOCK_MAPPING маппит GPT JSON → LandingBlock (15 блоков: accent/checklist/identification/price_table/text/subscriptions/special_formats/navigation/cta/faq); шаблон landing_page.html рендерит LandingBlock с fallback на JSON для старых записей; миграция 0008
